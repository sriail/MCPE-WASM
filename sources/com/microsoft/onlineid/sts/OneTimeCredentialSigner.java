...
public class OneTimeCredentialSigner {
    ...
    public String generateOneTimeSignedCredential(String appId) {
        ...
        return signedOneTimeCredential.appendQueryParameter(HashLabel, Base64.encodeToString(Cryptography.getInitializedHmacSha256Digester(new SecretKeySpec(this._sharedKeyGenerator.generateKey(SharedKeyGenerator.KeyPurpose.CredentialSignature, nonce), "HmacSHA256")).doFinal(signedOneTimeCredential.build().getEncodedQuery().getBytes(Strings.Utf8Charset)), 2)).build().getEncodedQuery();
    }
}
