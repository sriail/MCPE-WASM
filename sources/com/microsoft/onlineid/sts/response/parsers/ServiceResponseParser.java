...
import com.microsoft.onlineid.internal.log.IRedactable;
...
public class ServiceResponseParser extends BasePullParser {
    ...
    public ServiceResponseParser(XmlPullParser parser, byte[] sessionKey) {
        this(parser, sessionKey, (ISecurityScope) null);
    }

    /* access modifiers changed from: protected */
    public void onParse() throws IOException, StsParseException, XmlPullParserException {
        ...
        if (!TextUtils.isEmpty(encryptedHeader)) {
            ...
            Logger.info((IRedactable) new RedactableResponse("Decrypted service response header: " + decryptedHeader));
        ...
        if (this._error == null) {
            ...
            Logger.info((IRedactable) new RedactableResponse("Decrypted service response body: " + decryptedBody));
    ...
    private String decryptEncryptedBlob(String encryptedBlob) throws StsParseException {
        try {
            return new String(Cryptography.decryptWithAesCbcPcs5PaddingCipher(TextParsers.parseBase64(encryptedBlob), new SharedKeyGenerator(this._sessionKey).generateKey(SharedKeyGenerator.KeyPurpose.STSDigest, this._encKeyNonce)), Strings.Utf8Charset);
