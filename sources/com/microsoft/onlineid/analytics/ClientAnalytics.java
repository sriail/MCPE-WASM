...
public class ClientAnalytics implements IClientAnalytics {
    ...
    private ClientAnalytics(Context applicationContext) {
        ...
        reporter.setExceptionParser(new MsaExceptionParser(applicationContext, (Collection<String>) null));
    ...
    public static synchronized IClientAnalytics get() {
        IClientAnalytics iClientAnalytics;
        synchronized (ClientAnalytics.class) {
            if (Instance == null) {
                iClientAnalytics = new NopClientAnalytics();
            } else {
                iClientAnalytics = Instance;
            }
        }
        return iClientAnalytics;
    }
    ...
    public IClientAnalytics logEvent(String category, String action) {
        return logEvent(category, action, (String) null, (Long) null);
    }

    public IClientAnalytics logEvent(String category, String action, String label) {
        return logEvent(category, action, label, (Long) null);
    }
    ...
    public IClientAnalytics logException(Throwable throwable) {
        ...
        Assertion.check(z);
        send(new HitBuilders.ExceptionBuilder().setDescription(new MsaExceptionParser(this._applicationContext, (Collection<String>) null).getDescription(Thread.currentThread().getName(), throwable)).setFatal(false).build());
        return this;
    }
