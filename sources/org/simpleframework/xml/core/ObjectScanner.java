...
class ObjectScanner implements Scanner {
    ...
    private void field(Detail detail2) throws Exception {
        Iterator i$ = this.support.getFields(detail2.getType(), detail2.getOverride()).iterator();
        while (i$.hasNext()) {
    ...
    private void method(Detail detail2) throws Exception {
        Iterator i$ = this.support.getMethods(detail2.getType(), detail2.getOverride()).iterator();
        while (i$.hasNext()) {
            Contact contact = (Contact) i$.next();
            Annotation label = contact.getAnnotation();
            if (label != null) {
