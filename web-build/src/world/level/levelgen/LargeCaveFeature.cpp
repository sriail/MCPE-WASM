...
void LargeCaveFeature::addRoom(long seed, int xOffs, int zOffs, unsigned char* blocks, float xRoom, float yRoom, float zRoom) {
    addTunnel(seed, xOffs, zOffs, blocks, xRoom, yRoom, zRoom, 1 + random.nextFloat() * 6, 0, 0, -1, -1, 0.5f);
}

void LargeCaveFeature::addTunnel(long seed, int xOffs, int zOffs, unsigned char* blocks, float xCave, float yCave, float zCave, float thickness, float yRot, float xRot, int step, int dist, float yScale) {
    ...
    for (; step < dist; step++) {
        ...
        if (!singleStep && step == splitPoint && thickness > 1 && dist > 0) {
            addTunnel(random.nextLong(), xOffs, zOffs, blocks, xCave, yCave, zCave, random.nextFloat() * 0.5f + 0.5f, yRot - Mth::PI / 2, xRot / 3, step, dist, 1.0f);
            addTunnel(random.nextLong(), xOffs, zOffs, blocks, xCave, yCave, zCave, random.nextFloat() * 0.5f + 0.5f, yRot + Mth::PI / 2, xRot / 3, step, dist, 1.0f);
            return;
        }
        ...
        if (y0 < 1) y0 = 1;
        if (y1 > Level::DEPTH - 8) y1 = Level::DEPTH - 8;
...
void LargeCaveFeature::addFeature(Level* level, int x, int z, int xOffs, int zOffs, unsigned char* blocks, int blocksSize) {
    ...
    for (int cave = 0; cave < caves; cave++) {
        ...
        float yCave = (float)(random.nextInt(random.nextInt(Level::DEPTH - 8) + 8));
        ...
        for (int i = 0; i < tunnels; i++) {
            ...
            // Occasional larger tunnels (from LCE)
            if (random.nextInt(10) == 0) {
                thickness *= random.nextFloat() * random.nextFloat() * 3 + 1;
            }

            addTunnel(random.nextLong(), xOffs, zOffs, blocks, xCave, yCave, zCave, thickness, yRot, xRot, 0, 0, 1.0f);
        }
    }
}
